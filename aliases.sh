...
function create_alias() {
    ...
    local break_cast="$3"      # Whether it needs to "breakcast" (yes/no)
    local prefix_message="$4"  # Prefix message (optional)

    # Ensure required arguments are provided
    if [[ -z "$function_name" || -z "$alias_name" ]]; then
        echo "Usage: create_alias <function_name> <alias_name> [break_cast] [prefix_message]"
        return 1
    fi

    # Set default values for optional arguments
    break_cast=${break_cast:-"no"}
    prefix_message=${prefix_message:-""}

    # Construct the alias based on the value of break_cast
    if [[ "$break_cast" == "yes" ]]; then
        alias_command="alias $alias_name=\"$prefix_message; unalias $alias_name; $function_name\""
    else
        alias_command="alias $alias_name=\"$prefix_message; $function_name\""
    fi
    
    # Evaluate the alias command to create the alias
    eval "$alias_command" || echo "Failed to create alias for $alias_name"
}
        ...
        if [[ "$VIEW" == "group" ]]; then
            ...
            while IFS=: read -r group _ gid users; do
                if [[ $gid -ge $GID_MIN && $gid -le $GID_MAX ]]; then
                    ...
                    user_list=$(IFS=,; echo "${user_details[*]:-None}")
                    printf "%-25s | %-8s | %-50s\n" "$group" "$gid" "$user_list"
                fi
            done < <(getent group)
        elif [[ "$VIEW" == "user" ]]; then
